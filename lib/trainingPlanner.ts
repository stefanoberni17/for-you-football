...
export async function loadCarico(userId: string, isDeload: boolean, setRpe?: SetRpeRow[]): Promise<CaricoInfo> {
  ...
  if (!rpe) {
    try {
      const { data: logs } = await supabaseAdmin.from('training_set_logs').select('session_key, rpe')
        .eq('user_id', userId).gte('created_at', since).not('rpe', 'is', null).limit(600);
      rpe = (logs || []) as SetRpeRow[];
    } catch { rpe = []; }
  }
