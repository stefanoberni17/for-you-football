...
- **"Dove l'hai sentito?"** (migration 020, colonna `training_set_logs.sensazione`): dopo l'ultima serie degli esercizi con `sensazioni` nel catalogo (24 legati ai test: towel curls, toes up/down, affondo iso, wall sit, calf, copenhagen, nordic, plank…) il player mostra le opzioni; `lib/trainingAdapt.ts` conta le occorrenze e segnala un "(fastidio)"/"crampo" scelto ≥2 volte al planner e alla chat. Se la migration manca, il log si salva senza sensazione.
...
### Da fare
- [ ] **Migration 020 da applicare su Supabase** (`docs/migrations/020_training_set_logs_sensazione.sql`): colonna `sensazione` sui log per serie — finché manca, le sensazioni non vengono salvate (nessun errore).
