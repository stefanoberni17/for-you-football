...
- **Giornata a due fasi rifatta (20/9, Ste: "il flusso non funziona bene… si sblocca 4 ore dopo aver letto la prima parte"):** i 7 giorni `Tipo Pratica = giornata` (i G5 di W1-3 e W5-8). Prima: popup con i paragrafi della Pratica come "step", timer di 5' con l'anello "Osserva" al posto del Reset, avvio scritto solo alla fine del popup (chi lo chiudeva restava senza giornata), la sera nessuna missione sotto la domanda, chiusura possibile subito, e chi rispondeva la mattina dopo perdeva un giorno. Ora: (1) **avvio segnato SUBITO** (`avviaGiornata` → PUT) al tap su "Reset breve e vai" o "Vai senza Reset"; (2) il popup per le giornate è solo il **Reset breve**: 3 respiri col cerchio 4/6, `RESET_BREVE_SEC = 30`, step fissi, "Vai in giornata"; (3) **`GIORNATA_ATTESA_ORE = 4`** (`lib/constants.ts`): la riflessione si apre 4 h dopo l'avvio (`riflessioneAperta/riflessioneApreAlle` in `lib/dayUnlockLogic.ts`, `startedAt` = `created_at` della riga, esposto da `GET /api/giorno`); prima, la pagina mostra "Giornata in corso" con la missione (`pratica`) e l'ora ("La riflessione si apre alle 12:40"), la home dice "Giornata in corso · si apre alle…" e in rilettura l'ultima slide ha "Si chiude dalle HH:MM" al posto di "Ho fatto"; (4) la sera la domanda ha sopra un `<details>` "La missione di oggi"; (5) **time-gate sull'avvio**: `DayProgress.startedAt` (tutte le query selezionano `created_at`), `riferimentoChiusura` = avvio se precede la chiusura → chi risponde a colazione ha il G6 aperto lo stesso giorno; (6) niente Reset automatico dopo la chiusura serale (`DAY_COMPLETED_KEY` solo per gli altri tipi). Niente da cambiare su Notion (Durata Minuti ignorata per le giornate).
