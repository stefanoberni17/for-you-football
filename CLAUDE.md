...
- **Rombo a 10 punte** (`ROMBO_PUNTE`: palleggi, tiro/passaggio, push, pull, core, gambe, esplosività, velocità, resistenza, fascia) con `fatti/totali` per punta; radar Recharts nell'hub con asse fisso 0-100. **Punteggio ancorato ai livelli** (`punteggioLivelli` in `lib/trainingCatalog.ts`, unico per v1/v2/palestra): 40 alla soglia intermedio · 60 avanzato · 80 PRO · 100 un gradino oltre, verso rispettato per i tempi. `buildRombo` RICALCOLA dal valore con le regole correnti (`punteggioRisultato`; per i massimali usa `dettaglio.rapporto`), non dal `punteggio_calcolato` salvato: i massimali senza peso corporeo restano fuori dalla media (`nonValutabili`), i punti scala skill e l'AMRAP non entrano.
