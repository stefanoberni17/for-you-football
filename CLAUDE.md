...
**Fine seduta (Ste, 23/9, migration 026 — DA APPLICARE):** dopo l'ultimo esercizio la pagina seduta chiede il **voto 1-10 sulla seduta intera**, il **giudizio per blocco** (facile / giusto / duro, chips per ogni blocco della seduta) e la nota → `POST /api/training/complete { rpe, blocchi: [{id, nome, giudizio}], note }` (colonne `training_session_completions.rpe SMALLINT` e `feedback_blocchi JSONB`; il server ricava anche il vecchio `feedback` dal voto: ≤4 facile, ≥8 duro; se la migration manca salva come prima e risponde `skipped: 'migration_026'`). "Segna fatta senza voto" resta possibile. Uso: (1) **carico** — `calcolaCarico` usa il voto della seduta (`rpeFonte: 'seduta'`) PRIMA della media delle serie, poi feedback, poi 6; (2) **planner** — `loadFeedbackRecenti` (fail-soft) + `feedbackSeduteBlock` in `lib/trainingPlanner.ts` mettono nel prompt v1 e v2 la sezione "# FEEDBACK SEDUTE" (ultime 12: data, voto, blocchi con giudizio e id, nota) e la **regola 10 riscritta**: per ogni famiglia il giudizio sul blocco fatto l'ultima volta decide il codice (facile → successivo o short→full, giusto → stesso, duro / voto ≥ 8 / dolori → short o precedente, mai saltare). Prossimo passo: il server sceglie il codice da solo dai giudizi (memoria dei blocchi), i percorsi per famiglia dai programmi Everfit (`docs/everfit-programs.md`: Tecnica Base 12 settimane, Forza e Velocità 12, Fascia Foundation 8) e le scale per esercizio di kettlebell/corsa/palestra.
...
- [ ] Migration `026_session_feedback.sql` da applicare su Supabase (voto 1-10 e giudizio per blocco a fine seduta; senza, il completamento passa lo stesso ma voto e giudizi non si salvano)
