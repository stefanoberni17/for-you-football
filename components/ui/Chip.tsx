...
export default function Chip({
  children, selected = false, onClick, icon, disabled = false, size = 'md', tone = 'accent', className = '', showCheck = true, ariaLabel,
}: {
  ...
  return (
    <button
      ...
      className={[
        'inline-flex items-center justify-center gap-1.5 rounded-full border font-semibold transition-colors select-none',
        size === 'lg' ? 'h-12 px-5 text-body' : 'h-11 px-4 text-body-sm',
    ...
    >
      {selected && showCheck ? <Check size={16} strokeWidth={3} aria-hidden /> : icon}
      <span>{children}</span>
    </button>
  );
}
