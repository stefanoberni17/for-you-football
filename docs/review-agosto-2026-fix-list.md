...
>
> **STATO (13/08/2026):** tutti i fix nel CODICE sono applicati (questo branch). I fix su
> NOTION sono pronti da incollare, testo per testo, in `review-agosto-2026-notion-edits.md`
> (il connettore Notion della sessione era in sola lettura).

---

## 🔴 Priorità alta

- [ ] **W5-G4 — battezzare "Lo Stacco".** *(testo pronto → notion-edits §1)* L'app (Palestra, cassetta strumenti, Coach) chiama
...
- [ ] **W7-G4 — battezzare "L'Anticipo".** *(testo pronto → notion-edits §2)* Stesso problema: in W7 lo strumento si chiama solo
...
- [x] **Coach AI — contraddizione sul perdono di sé in W7.** ✅ FATTO: vale il Contesto (non si apre in W7; spostato a Season 2+ nel CHECK CASSETTO). Il CHECK CASSETTO nel prompt dice
  ...
  prompt. *(codice)*
- [ ] **W8-G4 + card Palestra — "tieni l'informazione, posa il peso".** ✅ card Palestra (`toolsCatalog.ts`) FATTA; resta W8-G4 su Notion *(→ notion-edits §3)*. Il canone (C06
...
- [ ] **W8-G7 — terza domanda gate.** *(testo pronto → notion-edits §4)* Il testo dice "se ti va, scrivi…" ma l'app richiede
...
- [ ] **Perché Funziona — togliere i termini inglesi.** ✅ style guide aggiornata (registro dichiarato); restano le 4 property su Notion *(→ notion-edits §2 e §8)*. Sono testi user-facing per 16-25enni:
...
- [ ] **DB Fondamenta — ordine principi 2 e 3.** *(→ notion-edits §10)* Fondamenta dice Presenza → Ascolto (2) →
...
- [ ] **DB Fondamenta — pulizia.** *(→ notion-edits §11)* Righe duplicate (Ascolto, Perdono, Lasciare Andare,
...
- [ ] **W8-G2 — valvola di sicurezza mancante.** *(testo pronto → notion-edits §5)* È l'unico giorno "che tocca" (fatto vs
...
- [ ] **W5-G1 — minuto di presenza pura.** *(testo pronto → notion-edits §6)* Il Piccolo Libro ("Pratica — Adesso", priorità
...
- [ ] **W6 — Nota Campo di G2 mai ripresa.** *(testo pronto → notion-edits §7)* W5-G3, W7-G3 e W8-G3 controllano la Nota Campo
  del G2 della loro settimana; W6-G3 no. Aggiungere il Testo Check. *(Notion)*
- [ ] **Durate incoerenti.** *(→ notion-edits §9)* G5 "giornata": W5/W6/W7 = 2 min, W8 = 5. Gate: W5/W6 senza
